...
function ZodiacComparison() {
  ...
  useEffect(() => {
    analytics.pageViewed('Zodiac Comparison Page')
  }, []);
  ...
  const calculateDates = async () => {
    ...
    if (astronomical !== astrological) {
      setIsLoading(true)
      try {
        const comparison = await getPersonalityComparison(astronomical, astrological)
...
export default ZodiacComparison 